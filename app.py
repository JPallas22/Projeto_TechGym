...
def acessar_aluno():
    if request.method == 'POST':
        ...
        if aluno:
            ...
            return render_template('acessar_aluno.html')  # <-- renderizar, não redirecionar
...
def cadastrar_horario():
    if request.method == 'POST':
        ...
        if hora not in horarios_validos.get(dia_semana, []):
            flash(f'O horário {hora} não é válido para {dia_semana}.', 'danger')
